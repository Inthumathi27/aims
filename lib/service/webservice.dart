...
class Webservice {
  ...
  Future<AwardBanner> fetchAwardData() async {
    ...
    Map<String, String> headers = {
      'Content-type': 'application/json',
      'Accept': 'application/json',
      // 'Authorization':"Bearer 1a32e71a46317b9cc6feb7388238c95d",
    };
    final response = await http.post(url, headers: headers,body:body ).timeout(
      Duration(seconds: TIME_DURATION),
      onTimeout: () {
    ...
    if (response.statusCode == 200) {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      prefs.setString("awardBannerData",response.body.toString());
      // If the server did return a 200 OK response,
      // then parse the JSON.
  ...
  Future<HolidayListData> fetchHolidayService() async {
    var url = Uri.parse(ApiConstants.holidayListUrl,);
    print(url);
    Map data = {
      'token':"1a32e71a46317b9cc6feb7388238c95d",
    };
    ...
    if (response.statusCode == 200) {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      prefs.setString("holidayListData",response.body.toString());
      // If the server did return a 200 OK response,
      // then parse the JSON.
      return HolidayListData.fromJson(jsonDecode(response.body));
    } else {
      // If the server did not return a 200 OK response,
      // then throw an exception.
      throw Exception('Failed to load album');
